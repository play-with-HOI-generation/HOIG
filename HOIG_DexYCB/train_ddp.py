...
class Train(object):
    def __init__(self):
        ...
        torch.cuda.set_device(self._opt.local_rank)

        data_loader_train = CustomDatasetDataLoader(self._opt, is_for_train=True, use_ddp=True)
        data_loader_test = CustomDatasetDataLoader(self._opt, is_for_train=False, use_ddp=True)

        self._sampler_train = data_loader_train.load_sampler()
        self._sampler_test = data_loader_train.load_sampler()
        ...
        self._model = ModelsFactory.get_by_name(self._opt.model, self._opt, use_ddp=True)
    ...
    def _train_epoch(self, i_epoch):
        ...
        self._sampler_train.set_epoch(i_epoch)
        for i_train_batch, train_batch in enumerate(self._dataset_train):
            ...
            # display terminal
            if (self._opt.local_rank == 0) and do_print_terminal:
                self._display_terminal(iter_start_time, i_epoch, i_train_batch, do_visuals)
                self._last_print_time = time.time()

            # display visualizer
            if (self._opt.local_rank == 0) and do_visuals:
                self._display_visualizer_train(self._total_steps)
                self._display_visualizer_val(i_epoch, self._total_steps)
                self._last_display_time = time.time()

            # save model
            if (self._opt.local_rank == 0) and (self._last_save_latest_time is None or time.time() - self._last_save_latest_time > self._opt.save_latest_freq_s):
    ...
    def _display_visualizer_val(self, i_epoch, total_steps):
        ...
        self._sampler_test.set_epoch(i_epoch)
