...
class HandRecoveryFlow(torch.nn.Module):
    ...
    def forward(self, src_img, ref_img, src_mano, ref_mano):
        ...
        input_G_src_hand = torch.cat([src_img * (1 - src_crop_mask_hand), src_cond_hand], dim=1)

        # tsf input
        input_G_tsf_obj = torch.cat([render_img_ref * (ref_crop_mask_hand - ref_crop_mask_bg), ref_cond_obj, ref_seg[:, 6:]], dim=1)
        input_G_ref_hand = torch.cat([render_img_ref * (1 - ref_crop_mask_hand), ref_cond_hand], dim=1)
...
class Trainer(BaseModel):
    ...
    def _create_generator(self):
        if self._opt.use_spade:
            return NetworksFactory.get_by_name(self._opt.gen_name, bg_dim=8, img_dim=3, obj_dim=3, img_cond_dim=3,
                                               obj_cond_dim=12, repeat_num=self._opt.repeat_num)
        else:
            return NetworksFactory.get_by_name(self._opt.gen_name, bg_dim=8, img_dim=12, obj_dim=12,
                                               repeat_num=self._opt.repeat_num)

    def _create_discriminator(self):
        return NetworksFactory.get_by_name('discriminator_patch_gan', input_nc=19,
    ...
    def set_input(self, input):
        ...
        with torch.no_grad():
            ...
            self._armask_src = src_mask
            self._armask_tsf = tsf_mask
        ...
        if self._opt.use_spade:
            ...
            fake_tsf_obj, fake_tsf_hand, fake_tsf_mask_bg, fake_tsf_mask_hand = \
                self._G.forward(self._input_G_bg, _input_G_src_obj_rgb, _input_G_tsf_obj_rgb,
                                ...
                                src_armask=self._armask_src, tsf_armask=self._armask_tsf)
        else:
            fake_src_bg, fake_tsf_bg, fake_src_obj, fake_src_hand, fake_src_mask_bg, fake_src_mask_hand, \
            fake_tsf_obj, fake_tsf_hand, fake_tsf_mask_bg, fake_tsf_mask_hand = \
    ...
    def _optimize_G(self, fake_src_imgs, fake_tsf_imgs, fake_masks_bg, fake_masks_hand):
        fake_input_D = torch.cat([fake_tsf_imgs, self._input_G_tsf_obj[:, 3:], self._input_G_tsf_hand[:, 3:], self._armask_tsf], dim=1)
    ...
    def _optimize_D(self, fake_tsf_imgs):
        tsf_cond = torch.cat([self._input_G_tsf_obj[:, 3:], self._input_G_tsf_hand[:, 3:], self._armask_tsf], dim=1)
    ...
    def load(self):
        ...
        # load G
        self._load_network(self._G, 'G', load_epoch, need_module=False)

        if self._is_train:
            # load D
            self._load_network(self._D, 'D', load_epoch, need_module=False)
